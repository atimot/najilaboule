...
| Hero フェード遅延 | dots `delay 0.8s` → h1 `delay 1s` → tagline `delay 1.5s` (各 duration 2s、`HERO_FADE` in `src/constants.ts`)。コンテンツは Loader の不透明オーバーレイの下に常時マウントされ、フェードは Loader 退場 (2.5s〜3.5s) と重なって完了する (LCP はフェード完了時点で記録されるため先行させる。delay は `LOADER_DURATION` から導出。reduced-motion 時は `REDUCED_FADE` の delay 0.3s / duration 0.5s) |
