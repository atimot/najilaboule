...
- **文言のハードコード禁止**。表示文字列は `src/content/ja.ts` に置き、`getCopy()` 経由で参照する。例外: 欧文の装飾ラベルのうち BOUTIQUE / ADDRESS / TEL / HOURS / ONLINE SHOP と画像 alt (`src/assets/alts.ts` に ja/en 併記)。章のラベル (MARMITE / L’ATTENTE / OBANZAI / RIZ ET SOUPE) と ACCÈS、章番号 (壱弐参四)、写真の注記 (`note`、現在は未使用) は `’` `È` や和文を字形サブセットに入れるため `ja.ts` に置く
