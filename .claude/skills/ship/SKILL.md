...
5. **lockfile を変更した場合のみ** wasm optional 依存の脱落チェック:
   ```bash
   node -e "const l=require('./package-lock.json');const need=['@emnapi/core','@emnapi/runtime'];const m=need.filter(p=>!Object.keys(l.packages).some(k=>k.endsWith('node_modules/'+p)));if(m.length){console.error('lockfile に欠落:',m.join(', '));process.exit(1)}console.log('wasm optional deps OK')"
   ```
